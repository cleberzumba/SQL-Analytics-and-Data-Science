...
DELETE FROM campanha_marketing
...
UPDATE campanha_marketing AS c
...
ALTER TABLE campanha_marketing
ADD COLUMN tem_outlier BOOLEAN DEFAULT FALSE;

-- Carrega a nova coluna (na clausula where sao as regras de identificação de outliers)
WITH stats AS (
    SELECT
        AVG(orcamento) AS avg_orcamento,
        STDDEV(orcamento) AS stddev_orcamento,
...
SELECT DISTINCT publico_alvo, publico_alvo_encoded
FROM campanha_marketing;
ORDER BY publico_alvo_encoded;
...
SELECT * FROM campanha_marketing;
